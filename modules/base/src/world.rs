...
CREATE TABLE IF NOT EXISTS _world (
    ...
    pub async fn notify(&self, conf: &NotifyConfig) -> anyhow::Result<()> {
        ...
        let res = sqlx::query(
            r#"
INSERT INTO _world (key, value, created_at, updated_at) VALUES ('git_tag', $1, $2, $3)
    ON CONFLICT (key)
    DO UPDATE SET value = $1, updated_at = $3
        WHERE _world.value != $1;
"#,
        )
        .bind(self.info.git_tag.to_string())
        .bind(now)
        .bind(now)
        .execute(&self.db)
        .await?;
        ...
        client
            ...
            .json(&json!({
                "content": format!("mode: {mode}\ngit tag: {git_tag}", mode = self.info.mode,git_tag = self.info.git_tag),
...
mod tests {
    ...
    async fn notify_test() {
        ...
        let conf = AppConfig::load("test", &info.mode).await.unwrap();
        println!("{:?}", conf);
