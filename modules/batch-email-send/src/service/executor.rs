...
use opxs_shared::message::batch::email_send::{EmailConfirmRequestParam, EmailSendJobMessage, EmailSendJobType};
...
impl Executor {
    pub async fn execute(&self, ms: &[EmailSendJobMessage]) -> anyhow::Result<()> {
        println!("---- 1 ----");
        for m in ms.iter() {
    ...
    async fn execute_one(&self, m: &EmailSendJobMessage) -> anyhow::Result<()> {
...
mod tests {
    ...
    use opxs_shared::message::batch::email_send::EmailSendJobStatus;
...
"#,
        ...
        let ms = vec![EmailSendJobMessage { id: job_id }];
