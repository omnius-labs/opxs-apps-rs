...
mod tests {
    ...
    async fn simple_test() {
        ...
        println!("{:?}", image_converter.convert_inputs.borrow().first().unwrap());
        assert_eq!(
            s3_client.get_object_inputs.borrow().first().unwrap().key,
            format!("in/{}", job_id).as_str()
        );
        assert_eq!(
            s3_client.put_object_inputs.borrow().first().unwrap().key,
